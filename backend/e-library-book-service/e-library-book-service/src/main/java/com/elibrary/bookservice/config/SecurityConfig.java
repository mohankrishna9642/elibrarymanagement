...
import java.util.Collections; // Keep this import if Collections.list is used, otherwise it can be removed
    ...
    public OncePerRequestFilter gatewayHeaderAuthFilter() {
        return new OncePerRequestFilter() {
            ...
            protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
                ...
                // --- NEW FIX: Concatenate strings for logger.debug to avoid overload ambiguity ---
                logger.debug("BookService: Incoming Request URI: " + request.getRequestURI());
                logger.debug("BookService: Headers from Gateway: X-User-ID=[" + userId + "], X-User-Email=[" + userEmail + "], X-User-Roles=[" + userRoles + "]");

                // --- Optional: Log ALL headers for deep debugging if needed ---
                // If uncommenting, ensure Collections is imported.
                ...
                if (StringUtils.hasText(userId) && StringUtils.hasText(userEmail) && StringUtils.hasText(userRoles)) {
                    ...
                    logger.debug("BookService: Authentication set in SecurityContextHolder for user: " + userEmail + " with roles: " + userRoles);
                } else {
